...
import { postLogin, getUserSelf, patchResetPwd, postLogout } from '../apis/userApi';
...
type Login = (account: string, password: string) => (({}) => mixed) => Promise<*>;
...
export const resetPwd: ResetPwd = withTokenAction(
  (account, legacyPassword, newPassword) => (dispatch, getState, token) =>
    patchResetPwd(token, {
      account,
...
export const logOut: LogOut = withTokenAction(userId => (dispatch, getState, token) =>
